(function () {
  ...
  function getProfileValue(profile, path) {
    ...
    for (const part of parts) {
      if (current === null || current === undefined ||
          !Object.prototype.hasOwnProperty.call(Object(current), part)) return undefined;
  ...
  function getResolvedProfileValue(profile, key) {
    const value = getProfileValue(profile, key);
    return value !== undefined ? value : profile?.[key];
  }
  ...
  function fill(profile, manual = false) {
    ...
    for (const el of fields) {
      ...
      const value = getResolvedProfileValue(profile, key);
